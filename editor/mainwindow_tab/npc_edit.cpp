...
void npc_edit::fill_data()
{
	...
	_data_loading = true;
	common::fill_ai_list(ui->ai_chain);

	ui->ai_chain->setCurrentIndex(game_data.game_npcs[dataExchanger->current_npc_n].gravity_level);
...
void npc_edit::on_npc_edit_tab_selectnpccombo_currentIndexChanged(int index)
{
    ...
    //std::cout << "SET graphic to '" << ui->npc_edit_tab_previewarea->get_graphicfile() << "'" << std::endl;

	int bg_pos = ui->bg_graphic_combo->findText(QString(game_data.game_npcs[index].bg_graphic_filename));
	//std::cout << ">>>>>>>> bg_file: '" << game_data.game_npcs[index].bg_graphic_filename << "', bg_pos: " << bg_pos << std::endl;
	...
	std::cout << "2. >> _data_loading: " << _data_loading << ", game_data.game_npcs[dataExchanger->current_npc_n].gravity_level: " << game_data.game_npcs[dataExchanger->current_npc_n].gravity_level << std::endl;
	ui->ai_chain->setCurrentIndex(game_data.game_npcs[dataExchanger->current_npc_n].gravity_level);
...
void npc_edit::on_pushButton_11_clicked()
{
	for (int i=0; i<GAME_MAX_OBJS; i++) {
		if (game_data.game_npcs[i].id == -1) {
			game_data.game_npcs[i].id = i;
            game_data.game_npcs[i].walk_range = RES_W/2;
            game_data.game_npcs[i].fly_flag = 0;
            QString temp_str = QString("[") + QString::number(i) + QString("]") + QString(" New NPC");
            sprintf(game_data.game_npcs[i].name, "%s", temp_str.toStdString().c_str());
...
void npc_edit::on_AddFrame_Button_clicked()
{
    ...
    // @204
    for (int i=0; i<GAME_MAX_OBJS; i++) {
        if (game_data.game_npcs[i].id == _npcedit_tab_selectednpc) {
            for (int j=0; j<ANIM_FRAMES_COUNT; j++) {
                int frame_type = ui->frame_list_selector->currentIndex();
                if (game_data.game_npcs[i].sprites[frame_type][j].used == false) {
                    std::cout << "ADD-SPRITE-FRAME: FOUND EMPTY SLOT AT [" << j << "]" << std::endl;
                    game_data.game_npcs[i].sprites[frame_type][j].used = true;
                    game_data.game_npcs[i].sprites[frame_type][j].duration = ui->npc_edit_tab_frameduration->value();
                    game_data.game_npcs[i].sprites[frame_type][j].sprite_graphic_pos_x = dataExchanger->getPalleteX();
                    game_data.game_npcs[i].sprites[frame_type][j].colision_rect.w = game_data.game_npcs[i].frame_size.width;
                    game_data.game_npcs[i].sprites[frame_type][j].colision_rect.h = game_data.game_npcs[i].frame_size.height;
                    game_data.game_npcs[i].sprites[frame_type][j].colision_rect.x = game_data.game_npcs[dataExchanger->current_npc_n].sprites_pos_bg.x;
                    game_data.game_npcs[i].sprites[frame_type][j].colision_rect.y = game_data.game_npcs[dataExchanger->current_npc_n].sprites_pos_bg.y;
                    reload_frame_list(ui->frame_list_selector->currentIndex());
                    return;
                }
            }
...
void npc_edit::reload_frame_list(int index)
{
	...
	/*
    while (ui->frameList_listWidget->count() > 0) {
		//printf(">>> Removing item %d from list, count: %d\n", i, ui->listWidget->count());
		QListWidgetItem* item_to_del = ui->frameList_listWidget->takeItem(0);
		delete item_to_del;
	}
	*/
	...
	for (int i=0; i<GAME_MAX_OBJS; i++) {
		if (game_data.game_npcs[i].id == _npcedit_tab_selectednpc) {
            ...
            // @204
            for (int j=0; j<ANIM_FRAMES_COUNT; j++) {
                if (game_data.game_npcs[i].sprites[index][j].used == true) {
                    int calc_pos_x = game_data.game_npcs[i].sprites[index][j].sprite_graphic_pos_x * game_data.game_npcs[i].frame_size.width;
                    ...
                    QString temp_str(game_data.game_npcs[i].name + QString(" (") + QString::number(game_data.game_npcs[i].sprites[index][j].duration) + QString(")"));
                    ...
                    image = image.copy(calc_pos_x, 0, game_data.game_npcs[i].frame_size.width, game_data.game_npcs[i].frame_size.height);
                    image = image.scaled(game_data.game_npcs[i].frame_size.width*2, game_data.game_npcs[i].frame_size.height*2);
                    item->setIcon(image);
                    item->setSizeHint(QSize(game_data.game_npcs[i].frame_size.width*2, game_data.game_npcs[i].frame_size.height*2+4));
    ...
    for (int i=0; i<GAME_MAX_OBJS; i++) {
        if (game_data.game_npcs[i].id == _npcedit_tab_selectednpc) {
            for (int j=ui->frameList_listWidget->currentRow(); j<ANIM_FRAMES_COUNT-1; j++) {
                game_data.game_npcs[i].sprites[ui->frame_list_selector->currentIndex()][j] = game_data.game_npcs[i].sprites[ui->frame_list_selector->currentIndex()][j+1];
            }
            // clear last sprite
            game_data.game_npcs[i].sprites[ui->frame_list_selector->currentIndex()][ANIM_FRAMES_COUNT-1].used = false;
            break;
        }
    }
    reload_frame_list(ui->frame_list_selector->currentIndex());
...
void npc_edit::on_frameUp_clicked()
{
    ...
    format_v_2_1::st_sprite_data temp = game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()-1];
    game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()-1] = game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()];
    game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()] = temp;
    reload_frame_list(ui->frame_list_selector->currentIndex());
}

int npc_edit::get_npc_index_from_id(int id)
{
    for (int i=0; i<GAME_MAX_OBJS; i++) {
        if (game_data.game_npcs[i].id == id) {
            return i;
        }
    }
    return -1;
...
void npc_edit::on_frameDown_clicked()
{
    ...
    if (game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()+1].used == false) {
        return;
    }
    format_v_2_1::st_sprite_data temp = game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()+1];
    game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()+1] = game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][ui->frameList_listWidget->currentRow()];
...
/*
void npc_edit::on_play_sprites_button_clicked()
{
    if (dataExchanger->playing_sprites == false) {
        dataExchanger->playing_sprites = true;
    } else {
        dataExchanger->playing_sprites = false;
    }
}
*/
...
{
    ui->sprite_duration_spinBox->setValue(game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][currentRow].duration);
	ui->sprite_colision_x->setValue(game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][currentRow].colision_rect.x);
	ui->sprite_colision_y->setValue(game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][currentRow].colision_rect.y);
	ui->sprite_colision_w->setValue(game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][currentRow].colision_rect.w);
	ui->sprite_colision_h->setValue(game_data.game_npcs[get_npc_index_from_id(_npcedit_tab_selectednpc)].sprites[ui->frame_list_selector->currentIndex()][currentRow].colision_rect.h);
}
...
void npc_edit::on_ai_chain_currentIndexChanged(int index)
{
	...
	std::cout << "3. >> _data_loading: " << _data_loading << ", game_data.game_npcs[dataExchanger->current_npc_n].gravity_level: " << game_data.game_npcs[dataExchanger->current_npc_n].gravity_level << ", index: " << index << std::endl;
	game_data.game_npcs[dataExchanger->current_npc_n].gravity_level = index; // @TODO use ai_type when system is working
}
