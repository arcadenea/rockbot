...
void EditorArea::paintEvent(QPaintEvent *) {
    ...
    if (dataExchanger->show_bg1 == true && bg1_filename.length() > 0) {
        ...
        if (filename.indexOf(".png") != -1) {
            if (dataExchanger->show_beaten == true) { // check if we need/can to show beaten background
                std::string bg1_beaten_filename = bg1_filename.substr(0, bg1_filename.length()-4);
                bg1_beaten_filename = std::string(EDITOR_FILEPATH) + "/data/images/map_backgrounds/" + bg1_beaten_filename + "_beaten.png";
                QFile file(bg1_beaten_filename.c_str());
                if (file.exists()) {
                    filename.clear();
                    filename.append(QString(bg1_beaten_filename.c_str()));
                }
            }
    ...
    if (dataExchanger->show_bg2 == true && bg2_filename.length() > 0) {
        ...
        if (filename.indexOf(".png") != -1) {
            ...
            if (dataExchanger->show_beaten == true) { // check if we need/can to show beaten background
                std::string bg2_beaten_filename = bg2_filename.substr(0, bg2_filename.length()-4);
                bg2_beaten_filename = std::string(EDITOR_FILEPATH) + "/data/images/map_backgrounds/" + bg2_beaten_filename + "_beaten.png";
                QFile file(bg2_beaten_filename.c_str());
                if (file.exists()) {
                    filename.clear();
                    filename.append(QString(bg2_beaten_filename.c_str()));
                }
            }
	...
	for (int i=0; i<MAX_MAP_NPC_N; i++) {
        ...
        if (npc_id >= GAME_MAX_OBJS || npc_id < 0) {
...
//            std::cout << "EditorArea::paintEvent #5.0.B - i[" << i << "], id_npc[" << npc_id << "] GOOD" << std::endl;
        ...
        if (!temp_image.isNull()) {
            ...
            QRectF source;
            if (stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].direction != ANIM_DIRECTION_RIGHT) {
	...
	for (int i=0; i<MAX_MAP_NPC_N; i++) {
        if (stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object != -1) {
            std::string filename = FILEPATH + "/data/images/sprites/objects/" + game_data.objects[stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object].graphic_filename;
			QPixmap temp_image(filename.c_str());
			if (temp_image.isNull()) {
                ...
                QRectF target(QPoint(stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].start_point.x*16*dataExchanger->zoom, stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].start_point.y*16*dataExchanger->zoom), QSize(game_data.objects[stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object].size.width*dataExchanger->zoom, game_data.objects[stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object].size.height*dataExchanger->zoom));
                QRectF source(QPoint(0, 0), QSize(game_data.objects[stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object].size.width, game_data.objects[stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object].size.height));
				painter.drawPixmap(target, temp_image, source);
			}
		}
	}
	...
	if (dataExchanger->editMode == EDITMODE_NORMAL) {
		if (dataExchanger->editTool == EDITMODE_NORMAL) {
			if (dataExchanger->layerLevel == 1) {
                ...
                stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile2.x = dataExchanger->getPalleteX();
                stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile2.y = dataExchanger->getPalleteY();
			...
			if (dataExchanger->layerLevel == 1) {
                ...
                stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile2.x = -1;
                stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile2.y = -1;
			} else if (dataExchanger->layerLevel == 3) {
...
/// @TODO: convert to new file-system ///
		...
		for (int i=0; i<MAX_MAP_NPC_N; i++) {
            if (stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].start_point.x == -1 && stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].start_point.y == -1) {
                std::cout << "reseting free-slot - i: " << i << ", id: " << stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].id_npc << ", x: " << stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].start_point.x << ", y: " << stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].start_point.y << std::endl;
                stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].id_npc = -1;
			}
		}
		...
		if (dataExchanger->editTool == EDITMODE_ERASER && found_npc != -1) {
			...
			for (int i=0; i<MAX_MAP_NPC_N; i++) {
                ...
                if (stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].id_npc == -1) {
                    ...
                    std::cout << ">> add npc to map - stage[" << dataExchanger->currentStage << "], map[" << dataExchanger->currentMap << "], slot[" << i << "], id[" << dataExchanger->selectedNPC << "], set_value[" << stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_npcs[i].id_npc << "]" << std::endl;
		...
		if (dataExchanger->editTool == EDITMODE_ERASER && found_object != -1) {
			...
			// search for free slot
			for (int i=0; i<MAX_MAP_NPC_N; i++) {
                std::cout << "searching for free-slot - i: " << i << ", id: " << stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object << ", x: " << stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].start_point.x << ", y: " << stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].start_point.y << std::endl;
                if (stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].id_object == -1) {
                    ...
                    stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].map_objects[i].direction = dataExchanger->npc_direction;
					repaint();
					return;
				}
			}
...
void EditorArea::fill_area() {
	...
	if (dataExchanger->layerLevel == 1) {
        ...
        originalX = stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile2.x;
        originalY = stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[editor_selectedTileX][editor_selectedTileY].tile2.y;
	} else if (dataExchanger->layerLevel == 3) {
	...
	for (i=editor_selectedTileX; i>=0; i--) {
		...
		for (j=editor_selectedTileY; j>=0; j--) {
			if (dataExchanger->layerLevel == 1) {
                ...
                searchX = stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[i][j].tile2.x;
                searchY = stage_data.stages[dataExchanger->currentStage].maps[dataExchanger->currentMap].tiles[i][j].tile2.y;
			} else if (dataExchanger->layerLevel == 3) {
